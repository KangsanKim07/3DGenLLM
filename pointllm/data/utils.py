...
def load_ulip2_objaverse_point_cloud(data_path, object_id, pointnum=8192, use_color=True):
    if not use_color:
        filename = f"{object_id}.pt"
        point_cloud = torch.load(os.path.join(data_path, filename)).permute(1, 0)
        point_cloud = point_cloud.numpy()
    else:
        filename = f"{object_id}.pt"
        point_cloud = torch.load(os.path.join(data_path, filename)).permute(1, 0)
        point_cloud[:, 3:] = point_cloud[:, 3:] / 255
        # point_cloud = torch.cat((point_cloud, torch.zeros(point_cloud.shape[0], 3)), dim=1)
        point_cloud = point_cloud.numpy()
        
        # x = point_cloud[:, 0]
        # y = point_cloud[:, 1]
        # z = point_cloud[:, 2]
        # color = np.array([[point_cloud[i, 3], point_cloud[i, 4], point_cloud[i, 4]] for i in range(len(point_cloud))])
